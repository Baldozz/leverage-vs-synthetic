...
replaces the recap of 24 September: the rotation now keeps the same market exposure as the loan portfolio, checked every month
(section 1), and the bank is assumed to call margin at 90 % of the lending value; every number below comes from these rules.*
...
- **Call prices** use Black–Scholes at the 5-year implied volatility and the 5-year Treasury yield of each day. Over the period
  ...
  every day; a call sold early at a month-end check is sold 1 vol point below the model price.
...
| Rotate into calls | 894 | 1,234 | 2,817 | 6,655 | 8,496 | 11,516 | 14,796 |

| Annualised return to today | Lowest | 5th pct | 25th pct | Median | 75th pct | 95th pct |
|---|---:|---:|---:|---:|---:|---:|
| Keep the loan | 8.8 % | 9.2 % | 12.0 % | 15.6 % | 17.1 % | 24.2 % |
| Rotate into calls | 8.6 % | 9.1 % | 12.1 % | 15.8 % | 17.3 % | 24.3 % |

- **The two portfolios earn the same.** Same start, same end, the rotation ends ahead on 68 % of all start dates and on **76 % of
  the settled ones**; its final NAV relative to keeping the loan is +1.0 % at the median of the settled starts (5th–95th
  percentile −4.4 % to +3.5 %; worst −7.4 % for an 8 September 2000 start, best +4.5 % for a 2 August 2013 start). The median
  annualised return is 15.8 % against 15.6 %, +0.1 points a year. On the 260 unsettled starts of 2020–25 the rotation is
  slightly behind (−0.6 % at the median, ahead on 27 %): they have paid premium and not yet collected.
- **Financing cost, in plain terms.** Keeping the loan costs the interest, which capitalises. Rotating costs the premium, which is
  partly recovered by the payoffs: from a 1997 start the rotation has paid USD 2,542 m of premiums and received 2,081 m of payoffs
...
- **What the rotation holds today.** At the median start, 68 % of its NAV is SPX, 18 % is call value (5th–95th percentile 12 % to
  29 %) and 17 % is T-bills — the payoffs not needed for replacements. From a 1997 start it is worth USD 12.2 bn against 12.1 bn
  for keeping the loan: 6.6 bn in SPX, 2.7 bn in calls, 3.0 bn in T-bills, in three calls.
...
- On that same start and day the rotation stood at **208 m** with 70 m of dry powder and no loan: 104 m of SPX and 104 m of
  calls — it had sold SPX for calls through 2008 to hold its exposure, so it went into the low with half its wealth in options.
- **The rotation cannot get a margin call** because it has no loan once the build is over. Its lowest NAV at any bottom was
  USD 198 m, for a start on 1 September 2000.
...
|---|---:|---|---|---:|---|---|---|---:|
| Oct 2002 | 265 | 241 / 390 | 290 / 419 | 100 % | 72 % (0) | 71 / 170 | 149 / 228 | 99 % |
| Mar 2009 | 600 | 142 / 293 | 198 / 367 | 100 % | 96 % (30) | −23 / 96 | 60 / 195 | 99 % |
| Mar 2020 | 1,176 | 413 / 1,772 | 416 / 1,882 | 95 % | 50 % (0) | 197 / 1,088 | 204 / 1,236 | 100 % |
| Oct 2022 | 1,309 | 503 / 3,003 | 508 / 3,141 | 96 % | 45 % (0) | 257 / 1,910 | 283 / 1,914 | 71 % |

- **In the two deep bear markets (2000–02 and 2007–09) the rotation was ahead on every start date in NAV** and had more dry
  powder on 99 % of them. At the 2009 low its median dry powder was USD 195 m against 96 m for keeping the loan; in the worst
  case 60 m against a margin call. The rotation's losses stop at the premium, while the loan keeps growing as the equity falls.
- **In the short falls of 2020 and 2022 the rotation's NAV was higher on 95–96 % of starts** and its dry powder on 100 % and
  71 %.
...
The two portfolios have the same exposure on every purchase day and after every roll, and the rotation's live exposure is back
inside ±10 % of the loan portfolio's on every month-end after the build: on this history there was always SPX left to fund the
shortfall, so no check was ever left short. Between month-ends the calls' delta moves with the market — up in a rally, down in a
fall — so the live exposure drifts: on the 1997 start it ran between 0.74× and 1.23× the loan portfolio's, and today it sits
near the top of the band on most starts (1.09×) after the rally. A typical settled start made 76 month-end sales and one
purchase over its life; the 1 vol point given up on the sales cost 4.6 m at the median. In a long fall the purchases turn stock
into options at the low, at the vol of the low: on the 24 March 2000 start the rotation sold SPX for calls through 2008 and went
into the 2009 low with half its wealth in options, which is why its dry powder there (70 m) is less than under the previous
rule (146 m) while its NAV is about the same. Those calls also take over the slots of the calls that expire afterwards, whose
rolls then find nothing to buy, so the weekly ladder of 52 calls thins to a handful after the first bear market (three calls
today on the 1997 start).
...
| | Keep the loan | Match the exposure (default) | Same, quarterly and T-bills only below the band | Same dollar delta (the rule before 24 Sep) | Same index units |
|---|---:|---:|---:|---:|---:|
| Median annualised return | 15.3 % | 15.4 % | 15.4 % | 17.7 % | 15.5 % |
| Rotation ahead of Keep on | | 76 % | 72 % | 99 % | 53 % |
| Worst / best same-start difference in final NAV | | −566 / +556 m | −696 / +593 m | −23 / +21,672 m | −2,176 / +498 m |
| Lowest NAV at the 2009 bottom | 142 m | 198 m | 211 m | 225 m | 234 m |
| Lowest NAV at the 2002 bottom | 241 m | 290 m | 306 m | 306 m | 306 m |
| Exposure ÷ Keep's today, median | 1.00 | 1.09 | 1.10 | 2.40 | 0.98 |
...
default (monthly check, SPX sold for calls when the T-bills run out) is ahead of the quarterly, T-bills-only variant on more
starts and with a smaller worst case, at the price of lower lows at the two deep bottoms, where it has bought calls into the
fall. Selling the early calls at the model mark rather than 1 vol point below it changes the median by a tenth of a percent.
...
| Dry powder in a deep bear market | Low, and in the worst case gone | Higher, though the calls bought at the low carry no lending value |
...
risk, and left more dry powder at the bottom of a severe crash.** Its price is a portfolio with less borrowing capacity after
...
A full daily record of every start date (NAV, dry powder, exposure, balance sheet) and the log of every roll and month-end trade
...
1. **The long-dated volatility is an estimate, not a dealer quote.** Bloomberg's implied volatility stops at 2 years. The
   ...
   calls bought near crash lows look cheaper than they were — and the rule buys calls in every long fall. **Before any decision,
   check current 5-year premiums with dealers. Their historical 5-, 7- and 10-year marks would replace the estimate.**
...
3. **Lombard terms are assumed.** The 75 % lending value and the margin call at 90 % of it are placeholders to confirm with the
   ...
   after rallies (3.3).
4. **The exposure is matched within ±10 %, monthly.** Between checks it drifts with the calls' delta (3.4). Differences smaller
   than the band should not be over-read. Selling SPX for calls at the low is a choice: the tool also runs the rule with the
   T-bills only (3.5).
...
6. **Not modelled:** taxes beyond dividend withholding (the SPX sold to fund replacements and month-end purchases would realise
   gains), currency risk (everything is in USD), and any use of the loan proceeds outside the two portfolios.
