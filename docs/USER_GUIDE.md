...
**Page 1's sidebar (the setup).** Today: SPX exposure (USD m), Lombard loan (USD m), spread over SOFR (bp), lending value of the SPX (%). The rotation: call tenor, weeks to complete the rotation (52; 1 = all on the start date). Advanced: dividend withholding tax, whether a call that expires worthless is replaced (default: not replaced; a call that ends in the money is always replaced on the same index units, paid from the payoff, then cash, then SPX), what happens to a payoff left after a roll (T-bills / SPX / more calls), lending value of the calls, the start-date grid.
...
1b. **NAV on the end day by start date**: the start date on the x axis; on top the NAV of each portfolio on the end day, one point per selected start; below, the net (Rotate − Keep on the same start) shaded blue where the rotation ends ahead and red where behind; dotted lines at the market peaks, dashed at the bottoms. Follows the start-year selection, the exclusion checkbox and *Held until*.
