...
**Page 1's sidebar (the setup).** Nothing runs until you press **Launch simulation** at the top of the page: the run is defined by the sidebar setup, the start-date grid, *Starts from* and *Held until*; the button is greyed out while none of them has changed since the last launch, and a note says when the page still shows the previous run. Today: SPX exposure (USD m), Lombard loan (USD m), spread over SOFR (bp), lending value of the SPX (%), margin call at LTV (%; 90 by default: the bank calls when the loan exceeds that share of the lending value, and the dry powder of both portfolios is that share of the lending value less the loan). The rotation: call tenor, weeks to complete the rotation (52; 1 = all on the start date). Advanced: dividend withholding tax; what an expiring call is replaced on (default *Keep's exposure*: the new call closes the gap between Keep's SPX value and what the rotation's holdings were bought to carry — its SPX plus each surviving call's slot, grown with the index since it was bought — so every replacement restores its own slot; paid from the payoff and the T-bills, then by selling SPX; a gap of zero or less buys nothing; or *the same dollar delta*: a new ATM call sized to carry the expiring call's exposure, notional = units × index ÷ delta, about twice the units; or *the same index units*); the band check — how often (monthly by default / quarterly / never), the band (±10 %), the unwind haircut on calls sold early (1 vol point), and what is bought below the band (default *ATM calls, SPX sold for them when the T-bills run out*; or ATM calls from the T-bills only; or SPX from the T-bills) — enabled with *Keep's exposure* only: at each check, an exposure above the band is cut by selling the most in-the-money calls to the band edge, one below it is raised by buying ATM calls, from the T-bills first and then by selling SPX for them; whether a call that expires worthless is replaced (default *replaced*; or *not replaced*, it lapses); what happens to a payoff left after a roll under the two earlier rules (T-bills / SPX / more calls; greyed out under *Keep's exposure*, where it stays in T-bills); lending value of the calls (0 %) and of the T-bills (90 %); the start-date grid. Under the two earlier rules every replacement is paid from the payoff, then the T-bills, then by selling SPX delta-for-delta — the exposure sold equals the exposure the calls bring, the excess over the premium goes to T-bills; when not even all the SPX can fund a replacement it is cut to what can be funded (the CSV counts these). The header line counts the band trades (up / down / partial) across the selected starts.
...
1c. **Dry powder on the end day by start date**: the same layout for dry powder — Keep's room before a margin call (90 % × 75 % × SPX − loan, red) with its LTV as dots on the right axis coloured yellow (far from a margin call) to orange (close to the call at 90 %; the colour bar sits under the chart, the highest LTV in the window is labelled in yellow with its start date), Rotate's dry powder (90 % × (75 % × SPX + 0 % × calls + 90 % × T-bills) − loan, blue); below, the net Rotate − Keep, blue where the rotation has more, red where less. Same labels for the lowest values. The NAV, dry-powder and exposure charts share fixed margins so their plot areas line up.
1d. **Exposure of the rotation relative to Keep's, over time**: the ratio of the rotation's live exposure (SPX + the calls' dollar delta) to Keep's SPX value, month by month across the selected starts alive on each date — median line and 5th–95th percentile band — with the band edges drawn (0.9 and 1.1 by default). Under *Keep's exposure* the ratio is 1 on every start day, inside the band on every funded check day, and back to 1 at a roll when no other call survives; between checks it drifts with the delta. Under the two earlier rules it shows how far the exposure runs from Keep's (the dollar-delta rule doubles it at every in-the-money expiry).
...
`.venv/bin/python scripts/export_trajectories.py` writes every start's trajectory to one long CSV, one row per start and month-end (`--sample W` for week-ends), with NAV, dry powder, the balance sheet and the live exposure of both portfolios in USD m and SPX with dividends rebased to 1 at the start. Default: weekly starts (`reports/trajectories_weekly_monthly.csv`, ≈ 250,000 rows, 26 MB, one minute); `--grid daily` runs every trading-day start (≈ 1.2 M rows, five minutes). The setup options mirror the sidebar (`--help`: `--roll target|delta|units`, `--rebalance`, `--band`, `--haircut`, `--below`); `--trade-log PATH` also writes the trade log (one row per roll decision and per tranche touched at a band check: exposure before and after, target, band edge, USD traded, notional, SPX, price, delta, strike, expiry, partial and cut flags). The per-start CSV of page 1 carries the counts (rolls skipped, rebalances up / down / partial), the USD sold and bought at rebalances, the unwind cost, both exposures today and their ratio, and the calls' share of the rotation's NAV.
`.venv/bin/python scripts/roll_rule_comparison.py` writes `reports/roll_rule_comparison.md`: the headline tables (value today, the bottoms) for the three roll rules side by side on the weekly grid, settled starts (to 22 Sep 2020) apart from the unsettled ones.
