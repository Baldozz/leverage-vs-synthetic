...
**Page 1's sidebar (the setup).** Nothing runs until you press **Launch simulation** at the top of the page: the run is defined by the sidebar setup, the start-date grid, *Starts from* and *Held until*; the button is greyed out while none of them has changed since the last launch, and a note says when the page still shows the previous run. Today: SPX exposure (USD m), Lombard loan (USD m), spread over SOFR (bp), lending value of the SPX (%). The rotation: call tenor, weeks to complete the rotation (52; 1 = all on the start date). Advanced: dividend withholding tax, whether a call that expires worthless is replaced (default: not replaced; a call that ends in the money is always replaced on the same index units, paid from the payoff, then cash, then SPX), what happens to a payoff left after a roll (T-bills / SPX / more calls), lending value of the calls (0 %) and of the T-bills (90 %), the start-date grid.
...
1b. **NAV on the end day by start date**: the start date on the x axis; on top the NAV of each portfolio on the end day, one point per selected start; below, the net (Rotate − Keep on the same start) shaded blue where the rotation ends ahead and red where behind; dotted lines at every market peak and dashed at every bottom up to the end day, always drawn. Follows the start-year selection, the exclusion checkboxes, *Held until* and the zoom slider, whose range runs from the launched start year to the end day. Each portfolio's lowest point is labelled with its value in its own colour, the start date, and the other portfolio's value on that start.
