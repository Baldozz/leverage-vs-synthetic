...
- `rolling_paths`: the same rows plus, for chosen path columns (room before a margin call of A, dry powder of B, NAVs), a wide table per column — value on the last trading day of each month (and the last day), one column per start, NaN before the start. The dry-powder page reads the 5th / 50th / 95th percentile across the starts alive on each date. The per-bottom tables of page 1 show, for the starts alive on the bottom day, each portfolio's own lowest, 5th / 25th / 50th / 75th / 95th percentile and highest of NAV and dry powder, Δ = the B cell − the A cell of the row (the two are usually different starts), and the count of starts where B > A on the same start (`test_app.py` recomputes both from `rolling_paths`). The worst-trajectory block takes the start with the lowest NAV that day in either portfolio and reads both on that start (Δ like for like), including the cost since the start from the running totals `interest_cum_A`, `premiums_cum_B`, `payoffs_cum_B` of `simulate` (cumulative sums of the daily interest, the premiums paid on build and roll days and the payoffs cashed at expiry; `test_cumulative_cost_columns_match_the_summary`).
