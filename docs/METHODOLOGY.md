...
- `rolling_paths`: the same rows plus, for chosen path columns (room before a margin call of A, dry powder of B, NAVs), a wide table per column — value on the last trading day of each month (and the last day), one column per start, NaN before the start. The dry-powder page reads the 5th / 50th / 95th percentile across the starts alive on each date.
