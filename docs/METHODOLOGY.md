...
- `rolling_paths`: the same rows plus, for chosen path columns (room before a margin call of A, dry powder of B, NAVs), a wide table per column — value on the last trading day of each month (and the last day), one column per start, NaN before the start. The dry-powder page reads the 5th / 50th / 95th percentile across the starts alive on each date. The per-bottom tables of page 1 show, for the starts alive on the bottom day, the dry powder of both portfolios on the starts at the lowest, 5th / 25th / 50th / 75th / 95th percentile and highest NAV of the portfolio with the lowest NAV that day (the start nearest each quantile), Δ on the same start, and the count of starts where dry powder B > A (`test_app.py` recomputes both from `rolling_paths`). On the end day the same layout gives the annualised return (NAV_end / NAV₀)^{1/years} − 1 over the starts. At a bottom the worst-trajectory block takes the start with the lowest NAV that day in either portfolio and reads both on that start (Δ like for like), including the cost since the start from the running totals `interest_cum_A`, `premiums_cum_B`, `payoffs_cum_B` of `simulate` (cumulative sums of the daily interest, the premiums paid on build and roll days and the payoffs cashed at expiry; `test_cumulative_cost_columns_match_the_summary`).
