...
- `rolling_starts`: the same setup put on at every start of a grid and run to a fixed date or for a horizon; one row per start (returns, least room / least capacity and their dates, interest, premiums, payoffs, end balance sheet).
