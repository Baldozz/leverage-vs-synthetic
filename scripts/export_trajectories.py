...
def main() -> None:
    ...
    ap.add_argument("--rebalance", choices=["quarterly", "monthly", "none"], default="monthly", help="target rule: when the exposure is checked against the band")
    ...
    ap.add_argument("--below", choices=["calls", "spx", "calls_spx"], default="calls_spx", help="target rule: below the band buy ATM calls from the T-bills, SPX from the T-bills, or calls from the T-bills then from SPX sold for them")
