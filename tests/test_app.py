...
def test_all_starts_page_weekly_grid() -> None:
    ...
    assert list(gfc.iloc[:11, 0]) == ["Worst trajectory", *detail, "Dry powder, same start, at the NAV percentiles of Keep the loan"] and gfc.iloc[18, 0] == "rotation ahead on" and len(gfc) == 19
    assert [c.split(":")[0] for c in gfc.iloc[11:18, 0]] == [f"at the {p} NAV" for p in pts]
    assert list(gfc.iloc[0, 1:]) == ["", "", ""] and list(gfc.iloc[10, 1:]) == ["", "", ""]   # the two section rows
    ...
    assert gfc.iloc[4, 1] == "—" and gfc.iloc[4, 2] == f"{da['call_val'] / m:,.0f} m (1 call alive of the 52 bought, on a notional of {da['call_notional'] / m:,.0f} m of index)" and gfc.iloc[4, 3] == pm(da["call_val"])
    ...
    assert gfc.iloc[9, 1] == f"interest {da['interest_cum_A'] / m:,.0f} m"
    assert gfc.iloc[9, 2] == f"premiums {da['premiums_cum_B'] / m:,.0f} m − payoffs {da['payoffs_cum_B'] / m:,.0f} m = {(da['premiums_cum_B'] - da['payoffs_cum_B']) / m:,.0f} m"
    assert gfc.iloc[9, 3] == pm(da["premiums_cum_B"] - da["payoffs_cum_B"] - da["interest_cum_A"]) and da["interest_cum_A"] == pytest.approx(pa.loc[:"2009-03-09", "interest_A"].sum())
    ...
    # dry powder, same start: the starts at Keep's NAV lowest / percentiles / highest, both strategies' dry powder on each of them
    picks = [na.idxmin(), *[(na - na.quantile(q)).abs().idxmin() for q in qs], na.idxmax()]
    assert [gfc.iloc[i, 0] for i in range(11, 18)] == [f"at the {p} NAV: {na[s_] / m:,.0f} m ({s_:%d %b %Y} start)" for p, s_ in zip(pts, picks, strict=True)]
    assert [gfc.iloc[i, 1] for i in range(11, 18)] == [f"{ra[s_] / m:,.0f} m" for s_ in picks] and [gfc.iloc[i, 2] for i in range(11, 18)] == [f"{db_[s_] / m:,.0f} m" for s_ in picks]
    assert [gfc.iloc[i, 3] for i in range(11, 18)] == [pm(db_[s_] - ra[s_]) for s_ in picks] and gfc.iloc[18, 3] == f"{int((db_ > ra).sum()):,} of 600 starts"
    assert gfc.iloc[11, 1] == "15 m" and gfc.iloc[11, 3] == gfc.iloc[8, 3]   # the lowest-NAV start is the worst trajectory: same dry powder Δ as in its block
    ...
    assert any(mk.value.startswith("**2022 bottom — 12 Oct 2022**") for mk in at.markdown) and len(at.table[4].value) == 19   # the 2022 correction has its table too
